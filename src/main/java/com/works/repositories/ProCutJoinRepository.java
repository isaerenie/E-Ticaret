...
public interface ProCutJoinRepository extends JpaRepository<ProCutJoin, Integer> {
    ...
    @Query(value = "select p.pid,p.cid,p.product_title,p.price,p.descripFtion,p.on_sale,c.category_title from product p inner join category c on p.cid=c.cid inner join photo ph on ph.pid=p.pid limit ?1,?2",nativeQuery = true)
